...
p1 <- plot_ly(fin.aid.long[level == "undergrad"], x = ~ay, y = ~avg_awd, type = 'bar') %>% 
      layout(title = "Returns by Moving Average Type - Undergrad")
p2 <- plot_ly(fin.aid.long[level == "graduate"], x = ~ay, y = ~avg_awd, type = 'bar') %>% 
      layout(title = "Returns by Moving Average Type - Grad")
subplot(p1, p2)
```
...
fin.aid.long %>%
  group_by(level) %>%
  do(p = my_plot(.)) %>%
  subplot(nrows = 1, margin = 0.04,  shareX = TRUE)
```
